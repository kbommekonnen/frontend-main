<template>
  <div class="flex flex-col space-y-1 h-full items-start bg-primaryBg p-1">
    <modal-header title="deposite form" @close="close" class="shadow-top" />
...
async function handleSave() {
  ...
  if (response.joiError) {
    bindJoiErrors(response.joi, "account amount paymentMode reason", setError)
  } else if (response.error) {
    errorMessage.value = response.message
  } else {
    openToast("deposited successfully")
    resetForm()
  }
