<template>
  <div class="flex flex-col h-full">
    ...
    >
      ...
      <template #discount="{ index, item }">
        <div v-if="item._id" class="flex space-x-2">
          <base-input
            type="number"
            class="max-w-fit"
            v-model="store.items[index].discount"
            padding="small"
          />
          <base-select
            placeholder="type"
            v-model="store.items[index].discountType"
            :items="discountTypeOptions"
            padding="small"
          />
        </div>
      </template>
      <template #tax="{ index, item }">
        <div v-if="item._id" class="flex space-x-2">
          <base-multi-select
            placeholder="taxes"
            v-model="store.items[index].taxes"
            :items="taxOptions"
            padding="small"
          />
        </div>
...
const { supportsDiscountAtItemLevel, supportsTax } = toRefs(
  usePreferencesStore(),
)
const sellItemsFormHeaders = [
  {
    label: "item",
    key: "_id",
  },
  ...
  },
  {
    ...
    type: "number",
    key: "discount",
    filter: () => supportsDiscountAtItemLevel.value,
  },
  {
    label: "tax",
    type: "number",
    key: "tax",
    filter: () => supportsTax.value,
  },
  {
    label: "sub total",
    type: "number",
