...
import EditAdjustment from "./components/Adjustments.vue"
...
export const deleteAdjustment = async function (id) {
  ...
  if (response) {
    eventBus.$emit("open-toast", {
      message: "Adjustment transaction deleted successfully",
    })
    eventBus.$emit("refresh")
  } else {
    eventBus.$emit("open-toast", {
...
export const confirmDelete = async function (id) {
  openModal({
    componentProps: {
      title: "Delete adjustment transaction",
      message: "Are you sure to delete adjustment transaction",
      api: {
        ...api.delete,
        params: [id],
      },
      toastMessage: "adjustment transaction deleted successfully",
      params: id,
    },
