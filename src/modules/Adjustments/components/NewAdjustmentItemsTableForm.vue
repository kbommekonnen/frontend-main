<template>
  <div class="flex flex-col h-full">
    <data-table
      :showHeader="false"
      :headers="adjustmentItemsFormHeaders"
      :items="store.items"
    ...
    <template #_id="{ value, item }">
      ...
      <template #quantityAvailable="{ value, item }">
        <div v-if="item._id" class="flex space-x-2">
          {{ value }}
        </div>
      </template>
      <template #newQuantityOnHand="{ index, item }">
        <div v-if="item._id" class="flex space-x-2">
          <base-input
            type="number"
            placeholder="new value"
            v-model="store.items[index].newQuantityOnHand"
            padding="small"
            bordered
          />
        </div>
      </template>
      <template #adjustedQuantity="{ index, item }">
        <div v-if="item._id" class="flex space-x-2">
          <base-input
            type="number"
            :positiveNumber="false"
            placeholder="adjusted value"
            v-model="store.items[index].adjustedQuantity"
            padding="small"
            bordered
          />
        </div>
...
<script setup>
import { adjustmentItemsFormHeaders } from "../headers"
