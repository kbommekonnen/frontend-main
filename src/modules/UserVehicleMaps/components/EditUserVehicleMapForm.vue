...
const props = defineProps({
  UserVehicleMap: { type: Object, default: () => {} },
})
...
init()
function init() {
	store.user = props.UserVehicleMap.user
	store.vehicle = props.UserVehicleMap.vehicle
}
async function handleUpdate() {
  ...
  const response = await putRequest({
    ...api.edit,
    params: [props.UserVehicleMap._id],
    data: store.form,
  })
  if (response.joiError) {
    ...
    openToast("uservehiclemap updated successfully")
    store.$reset()
    closeModal()
  }
  working.value = false
