<template>
  ...
  >
    ...
    <PerfectScrollbar class="flex-grow w-full flex flex-col space-y-0">
      ...
      <new-purchase-items-table-form
        class="py-4 px-1 flex-grow w-full flex space-x-1 bg-white"
        >{{ $t("items list") }}</new-purchase-items-table-form
      >
...
import NewPurchaseItemsTableForm from "./NewPurchaseItemsTableForm.vue"
...
async function handleSave() {
  ...
  if (response.joiError) {
    bindJoiErrors(
      response.joi,
      "name invoiceNumber issueNumber plateNumber orderNumber description",
      store.setError,
    )
