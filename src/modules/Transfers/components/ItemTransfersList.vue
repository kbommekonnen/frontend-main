<template>
  <div class="h-full flex flex-col bg-primaryBg space-y-1">
    ...
    <div class="flex-grow bg-white">
      <data-table
        :showHeader="false"
        :headers="itemTransfersHeaders"
        :api="api.getItemTransfers"
        :params="[id]"
        :handlers="handlers"
        class="h-full"
      ></data-table>
...
<script setup>
import { itemTransfersHeaders } from "../headers"
import api from "../api"
...
getRequest({ ...api.getItemTransfers, params: { id } }).then(({ items }) => {
  if (items) {
    itemTransfers.value = items
  }
})
</script>
