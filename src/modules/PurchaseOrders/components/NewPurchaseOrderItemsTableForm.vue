<template>
  <div class="flex flex-col h-full">
    <data-table
      :showHeader="false"
      :headers="purchaseItemsFormHeaders"
      :items="store.items"
    ...
    >
      <template #_id="{ value, item }">
        <base-select
          ...
          @focus="autoFocusSearchInput(true)"
          @blur="autoFocusSearchInput(false)"
        >
          <template #search>
        ...
        <div v-else>
          <base-accordion class="border border-t-0 border-primaryBg">
            <template #header>
              <div
                class="w-full flex space-x-2 px-1 rounded cursor-pointer "
              >
                <div class=" p-3">
                  <base-icon name="image" />
                </div>
      ...
      <template #quantity="{ index, item }">
        <div v-if="item._id" class="flex space-x-1">
          <base-input
            class="max-w-fit"
            type="number"
      ...
      <template #price="{ index, item }">
        <base-input
          ...
          v-if="item._id"
          type="number"
          v-model="store.items[index].price"
          padding="small"
        />
      </template>
      <template #subtotal="{ value, item }">
        <strong v-if="item._id" class="flex space-x-2">
          {{ value }}
        </strong>
      </template>
...
<script setup>
import { purchaseItemsFormHeaders } from "../headers"
...
let focusSearchInput = ref(false)
let searchInput = ref("")
let purchaseItem = ref("")
...
function autoFocusSearchInput(value) {
  focusSearchInput.value = value
}
</script>
